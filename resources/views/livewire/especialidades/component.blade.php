                        ...
                        <table class="table">
                            <thead>
                                <tr class="text-theme-1">
                                    ...
                                    <th class="border-b-2 dark:border-dark-5 whitespace-nowrap" >ESPECIALIDAD</th>
                            ...
                            <tbody>
                                @forelse ($especialidades as $especialidad) 
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        ...
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $especialidad->id }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $especialidad->nombre }}</h6>
                                            {{-- <small class="font-normal">{{ $MATERI->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>   
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $especialidad->materia->nombre }}</h6>
                                            {{-- <small class="font-normal">{{ $MATERI->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>   
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                @if ($especialidad->materia->count() < 1)
                                                    <button class="btn btn-danger text-white border-0"
                                                    onclick="destroy('especialidades','Destroy', {{ $especialidad->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                @endif
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $especialidad->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY ESPECIALIDADES REGISTRADAS </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
