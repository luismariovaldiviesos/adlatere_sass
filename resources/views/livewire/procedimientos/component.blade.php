                            ...
                            <tbody>
                                @forelse ($procedimientos as $procedimiento) 
                                    <tr class=" dark:bg-dark-1 {{ $loop->index % 2> 0 ? 'bg-gray-200' : '' }}">
                                        ...
                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $procedimiento->id }}</h6>

                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $procedimiento->nombre }}</h6>
                                            {{-- <small class="font-normal">{{ $MATERI->unidades->count() }} unidades en este edificio</small> --}}
                                        </td>

                                        <td class="dark:border-dark-5">
                                            <h6 class="mb-1 font-medium">{{ $procedimiento->materia }}</h6>
                                        ...
                                        <td class="dark:border-dark-5 text-center">
                                            <div class="d-flex justify-content-center">
                                                @if ($procedimiento->asuntos->count() < 1)
                                                    ...
                                                    onclick="destroy('procedimientos','Destroy', {{ $procedimiento->id }})"
                                                    type="button">
                                                        <i class=" fas fa-trash f-2x"></i>
                                                    </button>
                                                @endif
                                                <button class="btn btn-warning text-white border-0 ml-3"
                                                    wire:click.prevent="Edit({{ $procedimiento->id }})"
                                                    type="button">
                                ...
                                @empty
                                    <tr class="bg-gray-200 dark:bg-dark-1">
                                        <td colspan="2">
                                            <h6 class="text-center">    NO HAY PROCEDIMIENTOS REGISTRADOS </h6>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
